...
class Classifier(object):
    def __init__(self):
        ...
        rf_merge = RandomForestClassifier(
            ...
            max_features="sqrt", 
            random_state=42,
            n_jobs=-1  # Parallélisation ajoutée
        )
        svc_merge = SVC(
            kernel="rbf", 
            probability=True, 
            C=1.0, 
            gamma="scale",
            cache_size=1000,  # Augmenté de 200 à 1000 pour vitesse
            random_state=42
        )
    ...
    def _preprocess_X(self, X, fit=False, kept_idx=None, threshold=0.01):
        ...
        libsize = X_filtered.sum(axis=1, keepdims=True)  # Optimisé avec keepdims
        libsize[libsize == 0] = 1
    ...
    def fit(self, X_sparse, y):
        ...
        y_merged = y_arr.copy()
        mask_merge = np.isin(y_merged, list(merge_from))
        y_merged[mask_merge] = self.merged_label
    ...
    def predict_proba(self, X_sparse):
        ...
        not_nk_or_t_mask = ~nk_or_t_mask
        
        # For samples NOT predicted as merged class
        if not_nk_or_t_mask.any():
            indices = np.where(not_nk_or_t_mask)[0]
            pred_labels = y_pred_merge[indices]
            
            for i, pred_label in zip(indices, pred_labels):
        ...
        if nk_or_t_mask.any():
            ...
            for k, bin_class in enumerate(bin_classes):
                ...
                proba_final[nk_or_t_indices, class_idx] = proba_bin[:, k]
