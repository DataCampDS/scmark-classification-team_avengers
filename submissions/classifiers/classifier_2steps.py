"""
Two-stage hierarchical classifier for cell type classification.
"""
import numpy as np
...
def remove_genes(X, threshold=0.01):
    """
    Remove genes with variance below a threshold.

    Args:
        X (np.ndarray or csr_matrix): cells x genes matrix
        threshold (float): minimum variance threshold

    Returns:
        X_filtered: filtered matrix (same type as X)
        kept_idx: indices of kept genes
    """
    if issparse(X):
...
class Classifier(object):
    ...
    Stage 1: Classify into 3 merged classes
    Stage 2: Refine NK_or_T_cell into NK_cells vs T_cells_CD8+
    """
    ...
    def _preprocess_X(self, X, fit=False, kept_idx=None, threshold=0.01):
        ...
        Applies gene filtering, library size normalization, and log1p.

        Args:
            X: Feature matrix
            fit: Whether to fit (compute gene indices)
            kept_idx: Indices of genes to keep (if fit=False)
            threshold: Variance threshold for gene filtering

        Returns:
            X_processed: Preprocessed matrix
            kept_idx: Gene indices (only if fit=True)
        """
        ...
        if fit:
            ...
            if kept_idx is None:
                raise ValueError(
                    "kept_idx must be provided when fit=False"
                )
            X_filtered = X[:, kept_idx]
            return_idx = None
    ...
    def predict_proba(self, X_sparse):
        ...
        Returns:
            proba_final: Probability matrix (n_samples, n_classes)
        """
        ...
        if nk_or_t_mask.sum() > 0:
            ...
            X_bin_processed = self._preprocess_X(
                X_bin,
                fit=False,
                kept_idx=self.kept_idx_bin
            )
